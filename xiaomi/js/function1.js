	...
	function Zlunbo(pic,bigbannerBox,lis,colorArr,tuactiveZ,liactivebgColor,lunboTime,tuZ,lisColor,bannertushuliang,type=0,left1,right1){
	   if(type==0){	
		const tupian=$(pic);
      ...
      for(let k=0;k<tupian.length;k++){
        left.onclick=function(){
          clearInterval(t);
          if(k<1){
            k=tupian.length;
          }
          tupian[k].style.opacity=tuZ;
          li[k].style.background=lisColor;
          animate(tupian[k-1],{opacity:tuactiveZ},500);
          li[k-1].style.background=liactivebgColor;
        }
      }
				function move(){
