	...
	// left左边按钮
	// right右边按钮
	function Zlunbo(pic,bigbannerBox,lis,colorArr,tuactiveZ,liactivebgColor,lunboTime,tuZ,lisColor,bannertushuliang,leftbtn,rightbtn,type=0){
	   if(type==0){	
		const tupian=$(pic);
		const banner=$(bigbannerBox)[0];
		const li=$(lis);
		const color=colorArr;
		const left=$(leftbtn)[0];
		const right=$(rightbtn)[0];
		var flag=true;
