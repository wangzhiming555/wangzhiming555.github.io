	...
	function xuanxiangka1(btn,con){
		const btnh=parseInt(window.getComputedStyle(btn[0],null).height);
		const conh=parseInt(window.getComputedStyle(con[0],null).height);
		for(let i=0;i<btn.length;i++){
			for(let j=0;j<btn.length;j++){
				con[i].style.top=btnh;
			}
			btn[i].onmouseover=function(){
				animate(con[i],{top:-conh},500);
			}
			btn[i].onmouseout=function(){
				con[i].style.display="none";
			}
  ...
  function xslunbo(banhe){
      ...
      var t=setInterval(function(){
        ...
        animate(first,{height:0},1000,function(){
