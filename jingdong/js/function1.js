	...
	function Zlunbo(pic,bigbannerBox,lis,colorArr,tuactiveZ,liactivebgColor,lunboTime,tuZ,lisColor,bannertushuliang,left1,right1,type=0){
	   if(type==0){	
		const tupian=$(pic);
    ...
    function zylunbo1(bantu,banhe,left1,right1){
        const tu=$(bantu);
       const img=$(banhe)[0];
       const left=$(left1)[0];
       const right=$(right1)[0];
    ...
    window.onscroll=function(){
        ...
        for(let i=0;i<sec.length;i++){
            if(sob.scrollTop+0.5*CH>=sec[i].offsetTop){
                ...
                // btn[0].style.background=color[0];
